              ...
              <table class="table table-striped table-hover table-md" width=100%>
                ...
                <tr>
                  <td class="text-center">
                    <a href="#" class="btn btn-sm btn-warning" data-toggle="modal" data-target="#exampleModal-{{ $delivery->id }}">Edit</a>
                    <a href="{{ url('transmittals/' . $transmittal->id . '/delivery/delete/' . $delivery->id) }}" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure want to delete this record?')">Delete</a>
                  </td>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            <label>Remarks</label>
            <textarea name="delivery_remarks" id="delivery_remarks" class="form-control" cols="30" rows="6" required></textarea>
          </div>
        </div>
        <div class="modal-footer bg-whitesmoke br">
          ...
          <div class="btn-group">
            ...
            <button type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split d-none" data-toggle="dropdown" id="dd-button">
              {{-- <span class="sr-only">Toggle Dropdown</span> --}}
            </button>
            <div class="dropdown-menu bg-success">
              <button class="dropdown-item btn-success" type="submit" name="receive_button" value="receive"><i class="fas fa-file-signature"></i> Save & Received</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            <label>Status</label>
            <select name="delivery_status" id="delivery_status_{{ $delivery->id }}" class="form-control" onchange="editTemplate({{ $delivery->id }});">
          ...
          <div class="form-group">
            <label>Date</label>
            <input type="datetime-local" name="delivery_date" class="form-control" value="{{ date('Y-m-d\TH:i', strtotime($delivery->delivery_date)) }}" required>
          </div>
          <div class="form-group">
            <label>Remarks</label>
            <textarea name="delivery_remarks" id="delivery_remarks_{{ $delivery->id }}" cols="30" rows="6" class="form-control" required>{{ $delivery->delivery_remarks }}</textarea>
          </div>
        </div>
        <div class="modal-footer bg-whitesmoke br">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
          <div class="btn-group">
            <button type="submit" class="btn btn-primary">Save</button>
            <button type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split @if ($delivery->delivery_status == 'send') d-none @endif" data-toggle="dropdown" id="dd-button-{{ $delivery->id }}">
              {{-- <span class="sr-only">Toggle Dropdown</span> --}}
            </button>
            <div class="dropdown-menu bg-success">
              <button class="dropdown-item btn-success" type="submit" name="receive_button" value="receive"><i class="fas fa-file-signature"></i> Save & Received</button>
